...
d <- d |> filter(!(species %in% c("Argentina silus", "Argentina spp") & length_mm > 50)) |> 
...
d <- d |> 
  mutate(life_stage = ifelse(species %in% c("Ammodytidae", "Syngnathus rostellatus",
...
# Print details
print(nc_open(paste(covPath, "depth", "Mean depth natural colour (with land).nc", sep = "/")))
...
```{r, eval = F}
# FIXME: strange, the depth doesn't get added when I knit or evaluate chunks. Need to run this manually...
...
# Effect of depth [source: Copernicus]
