...
# FIXME: clean up this plot once we decide on the layout (collect axies, trim to show a specific year etc)
p_tempCovMap + p_chlCovMap + p_depthCovMap + p1 +
...
diagnostic <- data.frame(
  ...
  modelCall = c(
    "length_mm ~ temp_sc*species + year_ct*species + yday_ct", 
    "length_mm ~ temp_sc*species + year_ct*species + yday_ct with ST IID", 
    "length_mm ~ temp_sc*species + depth_sc*species + year_ct*species + yday_ct  with ST IID", 
    "length_mm ~ temp_sc*species + chl_sc*species + year_ct*species + yday_ct  with ST IID", 
    "length_mm ~ temp_sc*species  + depth_sc*species + chl_sc*species + year_ct*species + yday_ct  with ST IID"
  ), 
  loglink = c(
    m0_cv$sum_loglik,
    m1_cv$sum_loglik,
...
Go with model 3, spatiotemporal IID

```{r}

mod <- m3 # Paste here selected model
...
pred <- predict(mod, newdata = pred_grid %>%  mutate(species = "Clupea harengus")) # The ST and S random fields do not change with species
...
mean_coefs <- tidy(mod, conf.int = TRUE) %>%
  filter(term %in% c("temp_sc", "year_ct", "depth_sc",  "chl_sc")) %>% 
...
## Depth excluded due to insignificant effect across species and not present in the best performing parametrization. 
...
coefs_chl <- tidy(mod, conf.int = TRUE) %>%
  filter(grepl(":chl_sc", term)) %>% 
  bind_rows(mean_coefs %>% filter(term == "speciesAgonus cataphractus:chl_sc")) %>% 
  mutate(estimate2 = mean_coefs %>%
           filter(term == "speciesAgonus cataphractus:chl_sc") %>%
           pull(estimate) + estimate,
         ...
         conf.high2 = mean_coefs %>%
           filter(term == "speciesAgonus cataphractus:chl_sc") %>%
           pull(conf.high) + conf.high) %>% 
  mutate(species = str_remove(term, ":chl_sc"),
         species = str_remove(species, "species"))
...
p_chl <- coefs_chl %>% 
  ...
  geom_vline(xintercept = 0, alpha = 0.3, linetype = 2, linewidth = 0.25) +
  theme(axis.text.y = element_text(face = "italic")) + 
  labs(y = "Species", x = "Chlorophyll slope") + 
  scale_color_brewer(palette = "Dark2", direction = -1) +
  scale_fill_brewer(palette = "Dark2", direction = -1) +
  guides(color = "none", shape = "none") +
  geom_stripped_rows(aes(y = species), inherit.aes = FALSE) +
...
p_year + p_temp + p_chl +  plot_layout(axes = "collect")

ggsave(paste0(home, "/figures/coefs.pdf"), width = 17, height = 10, units = "cm")

```

### Conditional predictions of temperature, chl and year, with real values on the x and data, on 3 select species with contrasting trends.
...
pred_y <- predict(mod, newdata = nd_y, re_form = NA, re_form_iid = NA, se_fit = TRUE)
...
pred_t <- predict(mod, newdata = nd_t, re_form = NA, re_form_iid = NA, se_fit = TRUE)

# Chlorophyll effect
nd_c <- d %>%
  filter(species %in% c("Limanda limanda", "Maurolicus muelleri", "Sardina pilchardus")) %>% 
  group_by(species) %>%
  data_grid(chl_sc = unique(chl_sc)) %>% 
  ungroup() %>% 
  mutate(yday_ct = 0,
         year_ct = 10, # 2015
         year = as.numeric(year_ct + median(d$year)),
         temp_sc = 0, 
         chl = (chl_sc * sd(d$chl)) + mean(d$chl)) %>%
  filter(year %in% unique(d$year))

pred_c <- predict(mod, newdata = nd_c, re_form = NA, re_form_iid = NA, se_fit = TRUE)
...
lab_c <- tibble(species = c("Limanda limanda", "Maurolicus muelleri", "Sardina pilchardus"),
                lab = c("d)", "e)", "f)"))

p3 <- ggplot(pred_c, aes(chl, exp(est), color = species, fill = species)) + 
  geom_jitter(data = d %>% filter(species %in% c(nd_y$species)),
              aes(chl, length_mm), alpha = 0.5, height = 0, width = 0.3,
              size = 0.95, fill = NA, shape = 21) +
  geom_ribbon(aes(ymin = exp(est - 1.96*est_se),
                  ymax = exp(est + 1.96*est_se)), alpha = 0.4,
              fill = "grey", color = NA) +
  ...
  geom_text(data = lab_t, aes(label = lab), x = 0.5, y = Inf, vjust = 2,
            color = "grey20") +
  labs(x = "Chlorophyll (mg/m^3)", y = "Length (mm)") + 
  coord_cartesian(xlim = c(min(pred_t$temp), max(pred_t$temp))) +
  guides(color = "none", fill = "none") +
  facet_wrap(~species, scales = "free") + 
  scale_color_brewer(palette = "Accent") +
